...
"""
MOTOR DE OTIMIZAÇÃO CORRIGIDO - SYSTEM_FACIL.PY
================================================
Versão 2.0 - Fronteira Robusta + Múltiplas Fronts

CORREÇÕES:
✅ pop_size=300, n_generations=120 (espaço maior)
✅ Preserva 5 primeiras fronts (não só front 0)
✅ Inicialização com nichos (diversidade genética)
✅ Classificador robusto para poucas soluções
✅ Elitismo com crowding distance
✅ Mutação adaptativa por front
"""
...
class RobustNSGA2:
    ...
    def _initialize_population(self):
        """Inicialização com NICHOS para diversidade genética"""
        population = []
    ...
    def _generate_pool(self, strategy):
        ...
        if strategy == 'random':
            ...
            # Maximizar entropia posicional
            for _ in range(self.n_games):
                # Gerar jogo com distribuição uniforme nas posições
                game = []
                for pos in range(15):
                    # Cada posição tem distribuição diferente
                    low = max(1, pos * 1.5)
                    high = min(25, 25 - (14 - pos) * 1.5)
                    available = [d for d in range(int(low), int(high)+1) if d not in game]
                    if available:
                        game.append(np.random.choice(available))
                    else:
                        avail_all = [d for d in range(1, 26) if d not in game]
                        game.append(np.random.choice(avail_all))
                game = sorted(game)
                if tuple(game) not in seen:
                    seen.add(tuple(game))
                    pool.append(game)
    ...
    def run(self):
        ...
        print(f"   Fronts preservadas: {self.n_fronts_to_keep}")
        ...
        print(f"\n   ✅ Fronteira: {len(pareto_pools)} soluções ({len(final_fronts[0])} na front 0)")
    ...
    def _mutate(self, pool, generation):
        rate = 0.15 * (1 - generation / self.n_generations)  # Decai com geração
        ...
        for idx in indices:
            game = mutated[idx]
            strategy = np.random.choice(['drift', 'jump', 'restart'], p=[0.6, 0.3, 0.1])
...
def main():
    print("="*60)
    print("🧬 MOTOR ROBUSTO - SYSTEM_FACIL.PY")
    print("="*60)
